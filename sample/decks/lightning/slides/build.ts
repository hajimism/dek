// The build runs, the size is checked, and the one file lands on the right.
...
export default {
  ...
  draw(slide, { step, t }) {
    ...
    slide.querySelectorAll<HTMLElement>("[data-land]").forEach((el, i) => {
      const p = Math.min(1, Math.max(0, (t - LAND_AT - i * 300) / LAND_MS));
      const e = 1 - (1 - p) ** 3;
