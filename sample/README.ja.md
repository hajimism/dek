...
`decks/say-it-first/` は、dek を説明するのではなく売り込む 4 分のデッキです。課題、転換、主張、名前の由来、数字、呼びかけの順に進み、why-dek と共有しているのはプロジェクトだけです。デッキ自身の `theme.css` は、黒い舞台にひとつの紫、太いディスプレイ書体、右から拭うように入るページ送り。光と格子は疑似要素に描いているので、`lint --visual` が測るのは文字だけです。表紙で見出しが打ち込まれるところから、カウンタやダイヤルまで、入場の動きはすべて `slides/<id>.ts` が `t` から描くので、スクリーンショットにも PDF にも動画にも、同じ動きかその最終状態が出ます。`clock` のダイヤルはこのデッキ自身の `dek ls` の見積もりを、`moves` のビルドのカードは実際にビルドしたサイズを載せています。

## 3 つ目のデッキ

`decks/dek-ref/` は、dek にまだ無い機能 `dek ref` を売り込む 6 分のピッチデッキです。`dek ref` は、公開されている dek のデッキを `refs/` に取ってきて、読んだり描画したり、一枚借りたりできるようにする構想です。課題、気づき、プロダクト、仕組み、安心して使えること、ロードマップ、決めたいことの順に進みます。`theme.css` はほかの 2 つのどちらにも似せていません。クリーム色の紙に太い墨の輪郭、ぼかさない影、墨の文字の下に敷く派手な色（ライム、ピンク、空色、黄）。主張のページは青、最後は墨一色です。表紙には残り 2 つのデッキの実際のスクリーンショットを扇に広げ、`readonly` と `take` には why-dek の実際の `timing` スライドと `dek ls` の数字を載せています。`reskin` は、同じマークアップを、同じ名前のトークンに別々の値を入れて 2 回描いています。借りたスライドが自分の色になる、というこのピッチの要点そのものです。入場の動きは `slides/<id>.ts` が `t` から描きます。`close` の流れる帯は両端からはみ出すのが前提なので、疑似要素の文字で描いています。
