...
| コマンド | 役割 |
| --- | --- |
| `dek init [dir] [--deck NAME]` | `dir`（既定はカレント）にプロジェクトを作る。最初のデッキも作れる。`dek.toml`、`theme.css`、`.gitignore`、`.rumdl.toml`、`assets/`、`decks/`、`.dek/schema.json` を書く |
...
| `dek mv <old> <new>` | セクション id と HTML ファイルを改名。見出しの文言は触らない。宛先があれば拒否する |
...
| `dek sync` | 足りない骨格スライドを作り、`AGENTS.md` と `.dek/schema.json` を更新する。上書きしない |
...
| `dek build [--root-dist]` | HTML を 1 ファイル `decks/<deck>/dist/<deck>.html` に書く。`--root-dist` なら `<root>/dist/<deck>.html` |
