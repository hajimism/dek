...
sync は、あなたが手を入れたスライドには一切触れません。台本にビートを足しても、すでに書いた HTML は更新されません。紐付けは `data-step` の仕事です。骨格はビートの id を使うので、HTML を手で書き始める前にビートへ `{#id}` を付けておけば、あとからの挿入でスライドが壊れることはありません。
