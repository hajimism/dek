...
CLI はファイルを直接扱います。ブラウザが何を表示しているかを知る、あるいは変える必要のある 2 つのコマンド、`goto` と `current` だけが、起動中の開発サーバに HTTP で問い合わせます。サーバがなければ hint 付きで失敗します。
...
動画の撮影はトークを実時間で再生しません。各ビートは静止フレーム 1 枚と遷移に必要なフレームだけを生むので、描画時間はトークの長さではなく動きの量に比例します。
