...
`dek init` はプロジェクト直下に短い `AGENTS.md` を書き、`dek new` と `dek sync` がそれを最新に保ちます。三原則、規約、プロジェクトのテーマから抽出したクラス名・トークン・レイアウト、そして CLI 自身のリファレンスへのポインタ。100 行に収まります。それより細かいことは、必要になったときに CLI から引きます。
...
1. **結果を返すコマンドはすべて `--json` を受け付ける。** 成功は `{ "ok": true, ... }`、失敗は `{ "ok": false, "error": { "message", "path", "hint" } }` で終了コード 1。起動し続ける `dek` と `dek rehearse` だけが例外です。診断は SARIF。各診断は `severity` と、メッセージに含まれる値を `data` として持ち、失敗になるのは error だけです。エージェントに文章をパースさせません。
