...
ビートを再生せずに見せるページは、どれもそのビートの終わりを見せます。`dek shot`、`lint --visual`、PDF、それに `dek shot <a> --to <b>` のコマで出ていく側の枚です。スライドのスクリプトは動きの最後で描かれ、CSS のアニメーションとトランジションもすべて最後まで進めます。テーマに書いた入場の動きは、始まりのコマではなく、落ち着いた先の位置で測られ、撮られます。

`animation-iteration-count: infinite` のように終わらずに繰り返すアニメーションには、終わりがありません。静止画では最初のコマで止めるので、同じビートの静止画はどれも同じになります。プレイヤーはその終わりを待たずに進み、`dek video` はビートが動いているあいだ繰り返しを録り、そのあとは最初のコマで止めます。配布資料に載せたい姿を最初のコマにしてください。
