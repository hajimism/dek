...
| テーマの契約 | 未定義クラス、インラインスタイル、トークン、スコープ |
...
`DEK040`（読み辞書にない英単語）と `DEK042`（何かを見せるのに何も喋らないビート）は `voice/` のあるデッキにだけ適用されます。`DEK041`（ナレーションの長さが `duration` の予算から大きく外れている）は Timeline があるときだけです。3 つとも警告です。ライブ専用のデッキは lint が通れば完成で、声を足してもそれは変わりません。
