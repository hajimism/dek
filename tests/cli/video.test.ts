...
describe("dek video", () => {
    ...
    try {
      await withTempProject(
        ...
        async (root) => {
          ...
          expect(json.out.endsWith("demo.mp4")).toBe(true);
  ...
  test("writes one slide under .dek/video/", async () => {
    if (!ffmpegResolved()) {
      return;
    }
    const fake = await startFakeVoicevox();
    try {
      await withTempProject(
        ...
        async (root) => {
          ...
          expect(
            json.out.includes(`${join(".dek", "video", "demo", "intro.mp4")}`) ||
              json.out.endsWith("intro.mp4"),
          ).toBe(true);
          expect(await Bun.file(json.out).exists()).toBe(true);
        },
      );
